<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-visible shadow-sm sm:rounded-lg"><div class="p-6 text-gray-900" x-data="{ open: false, showDetailModal: false, selectedGuest: null }">
                    ...
                    <div class="mb-6 relative"><form method="GET" action="{{ route('admin.guestbook') }}">
                            <div class="flex flex-col md:flex-row gap-4">
                                <div class="relative flex-grow">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd" /></svg>
                                    </div>
                                    <input type="text" name="search" id="search" class="block w-full pl-10 pr-3 py-2 border-gray-300 rounded-md shadow-sm" value="{{ request('search') }}" placeholder="Cari berdasarkan ID atau Nama Tamu...">
                                </div>

                                <div class="relative">
                                    <button @click="open = !open" type="button" class="inline-flex items-center justify-center w-full md:w-auto px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 top-full mt-2 w-72 bg-white border border-gray-200 rounded-md shadow-lg z-50 p-4 space-y-4" style="display: none;">
                                        ...
                                        <div class="flex justify-between items-center pt-2">
                                            <a href="{{ route('admin.guestbook') }}" class="text-sm text-gray-600 hover:text-gray-900">Reset Filter</a>
                                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700">Terapkan</button>
                                        </div>
                                    </div>
                                </div>
                                <a href="{{ route('admin.guestbook.export', request()->query()) }}" class="inline-flex items-center justify-center w-full md:w-auto px-4 py-2 border border-green-300 shadow-sm text-sm font-medium rounded-md text-green-700 bg-green-50 hover:bg-green-100">
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            @if ($guest->tipe_kunjungan == 'janji_temu')
                                                JT{{ str_pad($guest->appointment_id, 2, '0', STR_PAD_LEFT) }}-{{ strtoupper(substr($guest->nama_tamu, 0, 2)) }}
                                            @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                                            No data matches your filter or search.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
            ...
            <div @click.away="showDetailModal = false" class="bg-white rounded-lg shadow-xl p-0 w-full max-w-lg mx-4">
                ...
                {{-- Template to avoid errors when 'selectedGuest' is still empty --}}
                <template x-if="selectedGuest">
                    <div class="p-6 space-y-4 text-sm">
                        <div class="grid grid-cols-3 gap-4">
                            <span class="font-semibold text-gray-500">ID Kunjungan</span>
                            <span class="col-span-2 text-gray-800 font-medium" x-text="selectedGuest.tipe_kunjungan === 'janji_temu' ? 'JT' + String(selectedGuest.appointment_id).padStart(2, '0') + '-' + selectedGuest.nama_tamu.substring(0, 2).toUpperCase() : 'OTS' + String(selectedGuest.id).padStart(2, '0') + '-' + selectedGuest.nama_tamu.substring(0, 2).toUpperCase()"></span>
                        </div>
                        <div class="grid grid-cols-3 gap-4">
